...
Start in `log` and tighten only once traffic confirms the shape. The recipe in
`vercel-api.md` is already `log` for that reason, and carries the detail of
which of its three `action` fields to change.
